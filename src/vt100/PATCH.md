...
The only change within the imported modules is in `perform.rs`: U+FFFD now
passes to `Screen::text`, while C1 characters retain the original unhandled
callback path. `mosh-client` rejects malformed UTF-8 before calling this parser.
...
U+FFFD and passes the same state, paint, and stock-server fixtures. Keep the
