...
The imported modules have three local changes. In `perform.rs`, U+FFFD passes
to `Screen::text` while C1 characters retain the original unhandled callback
path. In `screen.rs` and `cell.rs`, character width comes from the root crate's
shared stock-server compatibility policy instead of direct `unicode-width`
calls. The same policy is used before screen mutation, so admission, cell
storage, cursor movement, and paint agree. `mosh-client` rejects malformed
UTF-8 before calling this parser.
...
U+FFFD, supports the required width policy, and passes the same state, paint,
and stock-server fixtures. Keep the
