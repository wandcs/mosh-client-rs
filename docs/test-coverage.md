...
| Bounded local prediction | U, M, C, S | `src/prediction/tests.rs`, `tests/session.rs`, stock latency and isolation fixtures | Confirmation, divergence, expiry, capacity, conservative input classes, standard default, all three modes, adaptive hysteresis and glitch timing, per-Session ownership, latency, and authoritative convergence covered. Broader shell prediction is outside scope. |
