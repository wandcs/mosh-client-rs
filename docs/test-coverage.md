...
| LeanTTY MCRS-003 reproduces fatal local UDP I/O while the stock server and remote PTY remain alive | The physical WLAN case closed on `94f1322`. A later lid-close record identified `PermissionDenied` but could not identify send versus receive or raw errno. ADR 0012 therefore covers that category at both concrete I/O boundaries only after `Active`, with paced receive retry, same-socket authenticated recovery, initial and unlisted fatal errors, prompt cancellation, bounded close, and two-Session isolation. One fixed-revision physical lid-close rerun remains pending. |
